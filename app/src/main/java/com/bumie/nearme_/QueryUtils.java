...
public class QueryUtils {
    ...
    public static List<Datum> getPlaces(String url, OkHttpClient client){

        // Create an empty ArrayList that we can start adding colleges to
        List<Datum> mplaces = new ArrayList<>();

        Request request = new Request.Builder()
                .header("Authorization", "Bearer aqrcsFaHQ04sxiHAPHjfVnJy1LCd")
                .url(url)
                .method("GET", null)
                .build();
            ...
            public void onResponse(Call call, final Response response) throws IOException {
                if (!response.isSuccessful()) {
                    ...
                    try {
                        ...
                        // Create a JSONObject from the JSON response string
                        JSONObject baseJsonResponse = new JSONObject(text);
